(function(jtmpl, win) {
    ...
    var Ler = jtmpl.LEFT_DELIMITER,
    Rer = jtmpl.RIGHT_DELIMITER;
    ...
    jtmpl.clearAnnotation = function(str) {
        return String(str).replace(new RegExp(Ler + "??\\s*?//.*?\\s*?" + Rer, "g"), "").replace(new RegExp("<!--.*?-->", "g"), "").replace(new RegExp(Ler + "\\*.*?\\*" + Rer, "g"), "");
    }
    ...
    jtmpl.include = function(str) {
        return String(str == null ? "": str).replace(new RegExp(Ler + '\\s*?p(?:art)?\\s*?\\:\\s*?([\\w-]+)\\s*?' + Rer, 'g'),
        function(str, id) {
    ...
    jtmpl.load = function(str, callback) {
        ...
        if (refer = (/f(?:ile)?\:([A-Za-z0-9\u4E00-\u9FA5/ ? &.: =""] + ) / .exec(str) || [])[1]) {
            ...
            html = jtmpl.include(elem && elem.tagName && /input|textarea/ig.test(elem.tagName) ? elem.value: elem.innerHTML);
    ...
    var _javascript = function(str) {
        ...
        str = str.replace(new RegExp("(" + Ler + "\\s*?[html]*?\:??\\s*?\=\\s*?[^;|" + Rer + "]+?\\s*?)" + Rer, 'g'), "$1\;" + Rer).replace(new RegExp(Ler + '\\s*?h(?:tml)??\:\\s*?\=\\s*?([^;|' + Rer + ']+?);?\\s*?' + Rer, 'g'), "',typeof($1) === 'undefined' ? '' : jtmpl.encodeHTML($1),'").replace(new RegExp(Ler + '\\s*?\=\\s*?([^;|' + Rer + ']+?);\\s*?' + Rer, 'g'), "',typeof($1) === 'undefined' ? '' : $1,'");
