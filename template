(function(jtmpl, win){
			...
			* @ 其实处理方式，例如jq可以用回调处理
			**/
			jtmpl.render = function(str, el, data, callback){
				if (!str || typeof(str) != 'string' || !el || !data || !isObject(data)){
					return;	
				}
				...
				if (elem && elem.nodeName){
					elem.innerHTML = jtmpl.compile(str, data);	
				}else if (!!callback){
					callback(elem, jtmpl.compile(str, data));	
				}
			}
